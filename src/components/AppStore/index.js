...
class AppStore extends Component {
  state = {isSelectedTab: tabsList[0].tabId, selectedList: appsList}

  onChangeInput = event => {
    const selectedListByInput = appsList.filter(eachAppDetails =>
      eachAppDetails.appName
        .toLowerCase()
        .includes(event.target.value.toLowerCase()),
    )
    this.setState({selectedList: selectedListByInput})
  }

  selectedTab = tabId => {
    this.setState({isSelectedTab: tabId})
  }

  getfilteredAppsList = () => {
    const {isSelectedTab, selectedList} = this.state
    const requiredAppsList = selectedList.filter(
      eachAppDetails => eachAppDetails.category === isSelectedTab,
    )

    return requiredAppsList
  }

  render() {
    const {isSelectedTab} = this.state
    const filteredAppsList = this.getfilteredAppsList()
        ...
        <ul className="tabs-container">
          {tabsList.map(eachTab => (
            <TabItem
              ...
              isActive={isSelectedTab === eachTab.tabId}
            />
          ))}
        </ul>
        <ul className="app-items-container">
          {filteredAppsList.map(eachAppDetails => (
            <AppItem
              eachAppDetails={eachAppDetails}
              key={eachAppDetails.appId}
            />
          ))}
        </ul>
      </div>
